...
describe("the Predator preset", () => {
  ...
  it("ships sixteen presets, each with a distinct label", () => {
    expect(PRESET_DEFINITIONS).toHaveLength(16);
    expect(new Set(PRESET_DEFINITIONS.map((d) => d.label)).size).toBe(16);
  });
});
